...
# 1. BASE CODE ADALAH V7.22 (FINAL HYBRID MAC/WIN).
# 2. LOGIKA FFMPEG: OTOMATIS CARI DI FOLDER 'BIN' & AUTO-CHMOD (MAC).
# 3. FITUR: SYNC FOLDER TXT, SMART NAMING, INTERNET DATE CHECK.
# 4. COMPATIBILITY: WINDOWS (EXE) & MACOS (SOURCE RUN).
# 5. UPDATE LOCK: JIKA VERSI LAMA, APP TIDAK BISA DIBUKA (HANYA MUNCUL POPUP).
# ============================================================================
...
import random
import re 
import stat # Wajib untuk perizinan file di Mac
...
if platform.system() == "Windows":
    ...
    # Link Update Mac (Repo Baru: Intisari-Clips-Mac)
    GITHUB_EXE_URL = "https://github.com/faldi-intisariClipper/Intisari-Clips-Mac/releases/latest/download/IntisariClips-MacOS.zip"

# Link Cek Versi (Menggunakan Repo Mac agar satu komando)
VERSION_URL = "https://raw.githubusercontent.com/faldi-intisariClipper/Intisari-Clips-Mac/main/version.txt"
...
if platform.system() == "Windows":
    ...
    CREATE_NO_WINDOW = 0 # Di Mac nilainya 0
...
# --- LOGIKA BARU: DETEKSI FOLDER BIN (CROSS-PLATFORM) ---
def get_ffmpeg_path():
    # 1. Tentukan folder dasar aplikasi
    if getattr(sys, 'frozen', False):
        base_path = sys._MEIPASS
    else:
        base_path = os.path.dirname(os.path.abspath(__file__))

    # 2. Tentukan target file berdasarkan OS
    if platform.system() == "Windows":
        # Prioritas 1: Cek di folder bin/win/ffmpeg.exe
        candidate = os.path.join(base_path, "bin", "win", "ffmpeg.exe")
        if os.path.exists(candidate): return candidate
        
        # Prioritas 2: Cek sejajar dengan script
        ...
        # Prioritas 1: Cek di folder bin/mac/ffmpeg
        candidate = os.path.join(base_path, "bin", "mac", "ffmpeg")
        ...
        if os.path.exists(candidate):
            # OTOMATIS BERI IZIN EXECUTE (CHMOD +X) AGAR TIDAK ERROR
            try:
...
class DownloadTaskRow(ctk.CTkFrame):
    ...
    def start_download(self):
        ...
        # Pasang FFmpeg Path (kecuali jika default 'ffmpeg')
...
class UpdateBlocker(ctk.CTk):
    def __init__(self, new_ver):
        ...
        # --- KUNCI PINTU: JIKA DITUTUP, APP MATI ---
...
del "%~f0"
            ...
            # Logika Mac (Buka Browser ke Repo Baru)
            messagebox.showinfo("Update Tersedia", "Versi baru tersedia untuk macOS.\nSilakan download dan replace aplikasi lama.")
            webbrowser.open(GITHUB_EXE_URL.replace("/download/IntisariClips-MacOS.zip", "")) # Membuka halaman release
            sys.exit()
...
class LoginWindow(ctk.CTk):
    ...
    def open_wa(self):
        ...
        msg = f"Halo Admin, saya mau beli lisensi Intisari Clips.\nLicense ID saya: {current_input}"
...
# ============================================================================
# BOOTSTRAPING (PINTU GERBANG UTAMA)
# ============================================================================
...
if __name__ == "__main__":
    # 1. CEK UPDATE DULUAN SEBELUM MEMBUKA APLIKASI
    remote_v = get_remote_version()
    
    if is_update_required(remote_v):
        # JIKA UPDATE DIPERLUKAN: BUKA BLOCKER SAJA
        app_upd = UpdateBlocker(remote_v)
        app_upd.mainloop()
        # SETELAH POPUP DITUTUP, PYTHON AKAN BERHENTI DI SINI.
        # TIDAK AKAN LANJUT KE check_license_and_run()
    else:
        # JIKA AMAN: BARU JALANKAN APLIKASI UTAMA
